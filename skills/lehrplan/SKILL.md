---
name: lehrplan
description: "Set up and maintain an Austrian HTL teaching repository: identify the subject (Gegenstand) from the repo docs, sync the legal curriculum with RIS including a yearly amendment check (Novellen-Check), and extract the law into a complete subject file plus year-wise per-class extracts in UPPERCASE class folders. Use when: user mentions 'lehrplan', asks to 'prepare a teaching repo', 'check the Lehrplan for new Novellen', 'RIS sync', 'Lehrplan extrahieren', 'Jahrespläne neu ziehen', or mentions Unterrichtsvorbereitung on a repo following the lehrplan/ convention."
license: MIT
