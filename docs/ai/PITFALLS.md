...
- `~/.config/opencode/agents` is a symlink to the repo's `agents/` directory (same pattern as skills). opencode's agent loader scans `{agent,agents}/**/*.md` recursively with `symlink: true` — **every** `.md` in that tree becomes an agent, so never add README or auxiliary Markdown files to `agents/` (this is why the global instructions file lives at the repo root as `AGENTS.global.md`, not inside `agents/`).
- `~/.config/opencode/AGENTS.md` is a symlink to the repo's `AGENTS.global.md` — edit the repo file, never the symlink target path; the global instructions only load on the next session start after a change.
...
- SearXNG has **no native conditional fallback**: `weight` only re-ranks the merged results and all engines of a category run in parallel. The prioritized `brave` → `google` → `mwmbl,searchmysite` → `braveapi` chain is therefore implemented in `skills/searxng/searxng-search.sh`, not in the instance. Each tier is one `engines=` request; return the first tier with results.
